...
# Use the same base URL as conftest.py
...
class TestPersonaGroupsCRUD:
    ...
    @pytest.mark.asyncio
    async def test_create_persona_group(self, auth_client: AsyncClient, test_user):
    ...
    @pytest.mark.asyncio
    async def test_get_persona_groups_empty(self, auth_client: AsyncClient, test_user):
        """Test getting persona groups when none exist."""
        response = await auth_client.get(f"/groups?user_id={test_user['id']}")
    ...
    @pytest.mark.asyncio
    async def test_get_persona_groups_with_personas(self, auth_client: AsyncClient, test_user):
        """Test getting persona groups with included personas."""
        # Create a group
        group_response = await auth_client.post(
            f"/groups?user_id={test_user['id']}",
    ...
    @pytest.mark.asyncio
    async def test_update_persona_group(self, auth_client: AsyncClient, test_user):
    ...
    @pytest.mark.asyncio
    async def test_delete_persona_group(self, auth_client: AsyncClient, test_user):
        ...
        # Delete the group
        delete_response = await auth_client.delete(f"/groups/{group_id}")
        assert delete_response.status_code in [200, 204]

        # Verify it's gone
        get_response = await auth_client.get(f"/groups?user_id={test_user['id']}")
        assert len(get_response.json()["groups"]) == 0

    @pytest.mark.asyncio
    async def test_create_persona(self, auth_client: AsyncClient, test_user):
        ...
        # Create a persona
        persona_response = await auth_client.post(
            f"/groups/{group_id}/personas",
            json={
                "name": "Tech Lead",
    ...
    @pytest.mark.asyncio
    async def test_get_personas_by_group(self, auth_client: AsyncClient, test_user):
    ...
    @pytest.mark.asyncio
    async def test_update_persona(self, auth_client: AsyncClient, test_user):
    ...
    @pytest.mark.asyncio
    async def test_delete_persona(self, auth_client: AsyncClient, test_user):
        ...
        # Delete the persona
        delete_response = await auth_client.delete(f"/personas/{persona_id}")
        assert delete_response.status_code in [200, 204]

        # Verify it's gone
        get_response = await auth_client.get(f"/groups/{group_id}")
...
class TestPersonaGroupsValidation:
    ...
    @pytest.mark.asyncio
    async def test_max_groups_per_user(self, auth_client: AsyncClient, test_user):
    ...
    @pytest.mark.asyncio
    async def test_max_personas_per_group(self, auth_client: AsyncClient, test_user):
        ...
        for i in range(10):
            response = await auth_client.post(
                f"/groups/{group_id}/personas",
                json={"name": f"Persona {i+1}", "prompt": f"This is a valid prompt for persona {i+1} with sufficient length."}
            )
            assert response.status_code == 201
        ...
        response = await auth_client.post(
            f"/groups/{group_id}/personas",
            json={"name": "Persona 11", "prompt": "This is a valid prompt for persona 11 with sufficient length."}
        )
        assert response.status_code == 400
        assert "maximum" in response.json()["detail"].lower()

    @pytest.mark.asyncio
    async def test_group_ownership_validation(self, auth_client: AsyncClient, test_user):
        ...
        response = await auth_client.get(f"/groups?user_id={other_user_id}")
        
        assert response.status_code == 200
        assert len(response.json()["groups"]) == 0

    @pytest.mark.asyncio
    async def test_group_required_for_group_mode(self, auth_client: AsyncClient, test_user):
    ...
    @pytest.mark.asyncio
    async def test_group_exists_validation(self, auth_client: AsyncClient, test_user):
...
class TestGroupProcessingMode:
    ...
    @pytest.mark.asyncio
    async def test_single_mode_processing(self, auth_client: AsyncClient, test_user):
        ...
        # Fetch details to verify fields not in creation response
        details_response = await auth_client.get(f"/thoughts/{test_user['id']}/{thought_id}")
    ...
    @pytest.mark.asyncio
    async def test_group_mode_processing(self, auth_client: AsyncClient, test_user):
    ...
    @pytest.mark.asyncio
    async def test_group_mode_with_no_personas(self, auth_client: AsyncClient, test_user):
    ...
    @pytest.mark.asyncio
    async def test_persona_runs_created(self, auth_client: AsyncClient, test_user):
        ...
        # Submit thought
        thought_response = await auth_client.post(
            "/thoughts",
            json={
                "user_id": test_user["id"],
                "text": "Test thought",
...
class TestConsolidation:
    ...
    @pytest.mark.asyncio
    async def test_consolidated_output_structure(self, auth_client: AsyncClient, test_user):
        ...
        # Get thought details
        thought_response = await auth_client.get(f"/thoughts/{test_user['id']}/{thought_id}")
...
class TestCascadeDelete:
    ...
    @pytest.mark.asyncio
    async def test_delete_group_cascades_to_personas(self, auth_client: AsyncClient, test_user):
        ...
        # Delete the group
        await auth_client.delete(f"/groups/{group_id}")

        # Try to access the persona (should fail)
        # Check group personas instead
        group_response = await auth_client.get(f"/groups/{group_id}")
        assert group_response.status_code == 404

    @pytest.mark.asyncio
    async def test_delete_user_cascades_to_groups(self, auth_client: AsyncClient, test_user):
